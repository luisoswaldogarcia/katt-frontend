...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  useEffect(() => {
    getSession().then(s => {
      const g = s?.groups || []
      setCanManageUsers(g.includes('owner') || g.includes('superadmin'))
    })
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-katt-200 dark:border-katt-800">
          <div className="flex items-center gap-2">
            <img src={branding.appIcon} alt={branding.appName} className="w-10 h-10 rounded-full object-cover" />
            <span className="font-bold text-katt-600 dark:text-katt-300">{branding.appName}</span>
          </div>
